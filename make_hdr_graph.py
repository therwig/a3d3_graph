...
f, ax = plt.subplots()
for xi, yi, wi, l in zip(x, y, w, labels):
    ax.plot([xi], [yi], label=l, marker='o', markersize=nonlinear(wi))
    if 'LHC' in l:
        ax.text(xi*0.1, yi*30, l)
    elif l=='IceCube':
        ax.text(xi*0.1, yi*5, l)
    elif l=='XENON':
        ax.text(xi*0.1, yi*10, l)
    elif l=='DUNE':
        ax.text(xi*0.2, yi*20, l)
    elif l=='Neuro':
        ax.text(xi*0.2, yi*5, l)
#plt.legend()
ax.plot([1e-2], [1e18], label='1 TB/yr', marker='o', markersize=nonlinear(1e12), color='gray')
ax.plot([2e0], [1e18], label='1 PB/yr', marker='o', markersize=nonlinear(1e15), color='gray')
ax.plot([1e3], [1e18], label='1 EB/yr', marker='o', markersize=nonlinear(1e18), color='gray')
ax.text(0.23e-2, 0.3e19, '1 TB/yr', color='black',size=18)
ax.text(0.40e0, 0.7e19, '1 PB/yr', color='black',size=18)
